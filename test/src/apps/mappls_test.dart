...
void main() {
  group('Mappls Actions', () {
    ...
    test('view action builds web app link, Android intent, and fallback', () {
      ...
      expect(action.appLink.toString(), 'https://www.mappls.com/pom/28.6139,77.209');
      expect(action.androidIntentOptions.action, 'action_view');
      expect(action.androidIntentOptions.data, 'https://www.mappls.com/pom/28.6139,77.209');
    ...
    test('directionsWithCoords action builds route with title and mode', () {
      ...
      expect(
        ...
        'https://mappls.com/navigation?places=28.6129%2C77.2295%2CIndia+Gate&isNav=true&mode=walking',
      );
      expect(action.androidIntentOptions.action, 'action_view');
      expect(
    ...
    test('directionsWithCoords action supports default and alternate modes', () {
      ...
      final truckingAction = Mappls.directionsWithCoords(
        destination: const Coordinate(latitude: 28.6129, longitude: 77.2295),
        mode: MapplsTravelMode.trucking,
      );

      expect(defaultAction.destinationTitle, null);
      expect(defaultAction.mode, MapplsTravelMode.driving);
      expect(
        defaultAction.appLink.toString(),
        'https://mappls.com/navigation?places=28.6129%2C77.2295&isNav=true&mode=driving',
      );
      expect(
        bicyclingAction.appLink.toString(),
        'https://mappls.com/navigation?places=28.6129%2C77.2295&isNav=true&mode=biking',
      );
      expect(
        truckingAction.appLink.toString(),
        'https://mappls.com/navigation?places=28.6129%2C77.2295&isNav=true&mode=trucking',
      );
    });
