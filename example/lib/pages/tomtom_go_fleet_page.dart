...
/// Demonstrates deeplinks for TomTom Go Fleet.
class TomTomGoFleetPage extends StatefulWidget {
  /// Creates the TomTom Go Fleet example page.
  const TomTomGoFleetPage({super.key});

  @override
  State<TomTomGoFleetPage> createState() => _TomTomGoFleetPageState();
}

class _TomTomGoFleetPageState extends State<TomTomGoFleetPage> {
  ...
  final _titleController = TextEditingController(text: 'Fleet Yard');
  ...
  Widget build(final BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('TomTom Go Fleet')),
    body: SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () async => _deeplinkX.launchApp(TomTomGoFleet.open(fallbackToStore: _fallback)),
            child: const Text('Open TomTom Go Fleet'),
          ),
          const SizedBox(height: 24),
  ...
  Future<void> _launchDirections() async {
    ...
    await _deeplinkX.launchAction(
      TomTomGoFleet.directionsWithCoords(destination: destination, fallbackToStore: _fallback),
    );
  }
