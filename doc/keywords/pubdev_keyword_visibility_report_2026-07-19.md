...
The snapshot follows the `1.4.0` publish, preserves all 484 normalized queries from July 10, covers all 22 supported apps and seven stores found in `README.md` and `doc/apps/`, and adds four documented Threads search and topic-tag action phrases introduced in `1.3.9`.
...
- Globally throttled workers checkpointed every query and honored `Retry-After` with exponential backoff; all 488 unique queries completed and none were skipped.
...
pub.dev reports `deeplink_x` latest as `1.4.0`, published on `2026-07-17T14:21:44.204282Z`.
...
| Pub points | 160 / 160 |
| Likes | 13 |
| 30-day downloads | 1,124 |
| Topics | `deeplink`, `app-links`, `maps`, `navigation`, `open-store` |

The metadata API, score API, rendered package page, and rendered score page agreed at final collection.

## Executive Summary

- DeeplinkX ranks for 302 of 488 unique searches and is outside the top 100 for 186.
- Across the 484-query July 10 baseline, 52 improved, 13 regressed, 221 stayed at the same ranked position, 15 became newly ranked, 26 were lost, and 157 remained unranked.
- The `1.3.9` Threads expansion has 1 of four newly scanned search/topic phrases in the top 100; exact app/link/deeplink/open-from-Flutter visibility is reported in the matrix below.
- WhatsApp link intent remains a direct comparison with `whatsapp_unilink`; `whatsapp link` is #2 p1, while `whatsapp deeplink` is #1 p1.
- Regional navigation providers retain strong exact intent: Baidu Maps, 2GIS, and Yandex Maps link/deeplink positions are all visible in the top results.
- Broad task language remains the main gap: 1 of 10 related map-intent phrases, 0 of 6 WhatsApp task phrases, and 0 of 11 competitor-intent phrases rank in the top 100.
- Package quality signals improved without metadata changes: likes rose from 12 to 13 and 30-day downloads rose from 925 to 1,124.
...
| --- | ---: | ---: | ---: | ---: | --- |
| WhatsApp | #14 p2 | #2 p1 | #1 p1 | #1 p1 | `whatsapp`, `chat_bubbles`, `whatsapp_unilink`, `story_view`, `flutter_chat_bubble` |
| Telegram | #11 p2 | #2 p1 | #1 p1 | #1 p1 | `telegram`, `flutter_chat_bubble`, `televerse`, `appinio_social_share`, `social_sharing_plus` |
| Instagram | #10 p1 | #1 p1 | #1 p1 | #2 p1 | `instagram`, `story_view`, `zoom_pinch_overlay`, `story`, `pinch_zoom_release_unzoom` |
| Facebook | not in top 100 | #10 p1 | #4 p1 | #14 p2 | `facebook`, `firebase_auth`, `facebook_app_events`, `fade_shimmer`, `flutter_facebook_auth` |
| LinkedIn | #69 p7 | #14 p2 | #1 p1 | #9 p1 | `linkedin_login`, `social_sharing_plus`, `signin_with_linkedin`, `linkedin_login_android`, `flutter_animated_reaction` |
| Twitter | not in top 100 | #11 p2 | #2 p1 | #13 p2 | `twitter`, `firebase_auth`, `like_button`, `metadata_fetch`, `twitter_login` |
| Threads | #40 p4 | #2 p1 | #1 p1 | #3 p1 | `squadron`, `easy_isolate`, `flutter_pinned_shortcut_plus`, `syn_thread_safe_logger`, `flutter_chats_ui` |
| YouTube | #10 p1 | #1 p1 | #1 p1 | #1 p1 | `youtube`, `youtube_player_flutter`, `youtube_player_iframe`, `youtube_explode_dart`, `pod_player` |
| TikTok | #53 p6 | #3 p1 | #1 p1 | #9 p1 | `tiktok_events_sdk`, `appinio_social_share`, `drag_ball`, `tiktoklikescroller`, `tiktoken` |
| Pinterest | #12 p2 | #1 p1 | #1 p1 | #8 p1 | `flexbox_layout`, `overscroll_pop`, `custom_masonry_grid`, `masonry_gallery`, `pinterest_nav_bar` |
| Zoom | not in top 100 | #5 p1 | #1 p1 | #14 p2 | `zoom`, `extended_image`, `pinch_zoom`, `easy_image_viewer`, `zoom_pinch_overlay` |
| Slack | #29 p3 | #1 p1 | #1 p1 | #8 p1 | `slack`, `slack_notifier`, `slack_logger`, `flutter_chat_io`, `crash_reporter` |
| Google Maps | #23 p3 | #1 p1 | #1 p1 | #2 p1 | `google_maps`, `google_maps_flutter`, `google_maps_flutter_web`, `widget_to_marker`, `google_maps_flutter_android` |
| Apple Maps | #16 p2 | #1 p1 | #1 p1 | #2 p1 | `apple_maps`, `apple_maps_flutter`, `platform_maps_flutter`, `mapkit_js`, `apple_maps_place_picker` |
...
| Amap | #82 p9 | #1 p1 | #1 p1 | #1 p1 | `amap_flutter_location`, `ultra_map_place_picker`, `amap_map`, `fl_amap`, `xbr_gaode_navi_amap` |
| Moovit | #2 p1 | #1 p1 | #1 p1 | #1 p1 | `map_launcher`, `deeplink_x`, `iconify_flutter_plus`, `iconify_flutter`, `ubicons` |
| Baidu Maps | #2 p1 | #1 p1 | #1 p1 | #1 p1 | `map_launcher`, `deeplink_x`, `flutter_tabler_icons`, `iconify_flutter_plus`, `iconify_flutter` |
| 2GIS | #11 p2 | #1 p1 | #1 p1 | #3 p1 | `dgis_map_kit`, `dgis_flutter`, `flutter_echarts`, `geoengine`, `dgis_map_platform_interface` |
| Yandex Maps | #8 p1 | #1 p1 | #1 p1 | #1 p1 | `yandex_maps_mapkit_lite`, `yandex_maps_mapkit`, `yandex_maps_navikit`, `yandex_js_maps`, `yandex_map_desktop` |
...
| `deep link package` | #10 p1 | `deep_link_manager`, `flutter_facebook_app_links_spm`, `djangoflow_deep_link_interface`, `app_wide_search`, `djangoflow_app_links` |
| `deep link plugin` | #29 p3 | `flutter_branch_sdk`, `flutter_facebook_app_links`, `stack_deferred_link`, `link_bridge`, `eimzo_flutter` |
| `deep linking` | not in top 100 | `go_router_builder`, `go_router`, `auto_route`, `kiwi`, `zenrouter` |
...
| `deeplink flutter package` | #7 p1 | `deep_link_manager`, `djangoflow_deep_link_interface`, `my_deep_link_sdk`, `smart_deeplink_router`, `toropass_client` |
| `deeplink launcher` | not in top 100 | `webf_deeplink`, `flutter_rustore_billing`, `sojo_link`, `eimzo_flutter`, `lego_cli` |
| `deeplink package` | #8 p1 | `override_api_endpoint`, `deep_link_manager`, `djangoflow_deep_link_interface`, `my_deep_link_sdk`, `smart_deeplink_router` |
| `deeplink plugin` | #22 p3 | `flutter_branch_sdk`, `flutter_deep_links`, `google_ads_deferred_deep_link`, `xy_umeng`, `urlynk_flutter` |
...
| `app links` | #6 p1 | `app_links`, `whatsapp_unilink`, `flutter_branch_sdk`, `flutter_share`, `flutter_facebook_app_links` |
| `app-links` | #6 p1 | `app_links`, `whatsapp_unilink`, `flutter_branch_sdk`, `flutter_share`, `flutter_facebook_app_links` |
| `custom url scheme` | #67 p7 | `appscheme`, `app_links`, `link_bridge`, `flutter_dominant_color_container`, `uni_links3` |
| `custom url schemes` | #36 p4 | `app_links`, `flutter_branch_sdk`, `link_bridge`, `appscheme`, `flutter_deep_links` |
| `intent launcher` | not in top 100 | `intent_launcher`, `url_launcher`, `android_intent_plus`, `maps_launcher`, `external_app_launcher` |
| `ios url scheme` | not in top 100 | `appscheme`, `app_links`, `ios_native_utils`, `link_bridge`, `uni_links3` |
| `scheme launcher` | not in top 100 | `url_launcher`, `maps_launcher`, `external_app_launcher`, `open_mail_launcher`, `launchify` |
| `universal link` | not in top 100 | `app_links`, `whatsapp_unilink`, `flutter_branch_sdk`, `link_bridge`, `flutter_link_nav` |
| `universal links` | not in top 100 | `universal_links`, `app_links`, `whatsapp_unilink`, `flutter_branch_sdk`, `link_bridge` |
| `url scheme` | not in top 100 | `appscheme`, `dart_naver_web_tools`, `android_scheme_search`, `app_links`, `url_launcher` |
| `url schemes` | #55 p6 | `url_launcher`, `app_links`, `flutter_branch_sdk`, `telegram`, `link_bridge` |

### Open and Launch External Apps

| Keyword | Rank | Top visible packages ahead or around it |
| --- | ---: | --- |
| `app installed flutter` | #54 p6 | `appcheck`, `installed_apps`, `app_launcher`, `open_mail`, `flutter_install_referrer` |
| `check app installed` | #63 p7 | `appcheck`, `check_app_version`, `device_apps_plus`, `app_install_checker`, `app_data_usage` |
| `external app` | #44 p5 | `external_app_launcher`, `dropbox_client`, `ensemble_ts_interpreter`, `app_activity_launcher`, `external_app_launcher_plus` |
| `external app launcher` | not in top 100 | `external_app_launcher`, `app_activity_launcher`, `external_app_launcher_plus`, `app_launcher_packagename`, `launchify` |
| `flutter app launcher` | not in top 100 | `icons_launcher`, `flutter_launcher_icons`, `dynamic_app_icon_flutter_plus`, `external_app_launcher`, `flutter_app_badge` |
| `flutter launch app` | #82 p9 | `icons_launcher`, `flutter_launcher_icons`, `appcheck`, `external_app_launcher`, `launch_app_store` |
| `flutter open app` | #19 p2 | `flutter_open_app`, `open_file`, `serverpod`, `iconify_flutter_plus`, `adapty_flutter` |
...
| `launch external app` | #25 p3 | `launchexternalapp`, `external_app_launcher`, `app_activity_launcher`, `external_app_launcher_plus`, `app_launcher_packagename` |
| `launch installed app` | #42 p5 | `appcheck`, `app_launcher`, `open_mail_launcher`, `mappls_map_launcher`, `apps_utils` |
| `open app` | #23 p3 | `open_app`, `open_file`, `serverpod`, `iconify_flutter_plus`, `adapty_flutter` |
...
| `fallback to store` | #2 p1 | `flutter_in_store_app_version_checker`, `deeplink_x`, `abcx3_store_devtool`, `smart_app_update_flutter`, `billing_country` |
| `google play link` | #2 p1 | `app_device_integrity`, `deeplink_x`, `flutter_install_app_plugin`, `flutter_update_checker`, `flutter_update_checker_plus` |
...
| `open play store` | #9 p1 | `flutter_app_store`, `in_app_update_flutter`, `store_redirect`, `launch_app_store`, `open_appstore` |
| `open store` | #4 p1 | `open_store`, `geoflutterfire2`, `app_review`, `deeplink_x`, `flutter_geofire` |
| `play store deeplink` | #1 p1 | DeeplinkX leads |
| `redirect to store` | #16 p2 | `store_redirect`, `store_redirect2`, `simple_auth_flutter`, `dio_redirect_interceptor`, `flutter_app_update_lib` |
| `smart fallback` | #13 p2 | `flutter_smart_cache_manager`, `smart_localize`, `ai_smart_translate`, `locale_chain`, `smart_image` |
...
| `directions` | #96 p10 | `map_launcher`, `appinio_swiper`, `marquee`, `custom_rating_bar`, `loop_page_view` |
| `flutter maps launcher` | not in top 100 | `map_launcher`, `maps_launcher`, `launchify`, `mappls_map_launcher`, `launcher_utils` |
...
| `map launcher` | not in top 100 | `map_launcher`, `maps_launcher`, `flutter_launcher_plus`, `mappls_map_launcher`, `maplauncherplus` |
| `maps` | #38 p4 | `maps`, `map_launcher`, `maps_toolkit`, `google_maps_flutter`, `maplibre_gl` |
| `maps launcher` | not in top 100 | `maps_launcher`, `map_launcher`, `launchify`, `mappls_map_launcher`, `launcher_utils` |
...
| `navigation` | #64 p7 | `navigation`, `smooth_sheets`, `go_router_builder`, `auto_route_generator`, `curved_navigation_bar` |
| `open map` | #39 p4 | `maps_launcher`, `location_picker_flutter_map`, `open_street_map_search_and_pick`, `open_location_picker`, `flutter_location_search` |
...
| `turn by turn navigation` | #41 p5 | `flutter_mapbox_navigation`, `vietmap_flutter_navigation`, `nb_navigation_flutter`, `situm_flutter`, `flutter_mapbox` |
...
| `social app` | not in top 100 | `appinio_social_share`, `authentication_buttons`, `flutter_feed_reaction`, `custom_social_share`, `launchify` |
...
| `social deep link` | not in top 100 | `chottu_link`, `app_linkster`, `flutter_tabler_icons`, `bit_2_connect_sdk`, `uicons_pro` |
| `social deeplink` | not in top 100 | `sojo_link`, `archipelago_cli`, `firebase_dynamic_links_fixed`, `bit_2_connect_sdk`, `keeplist_social` |
| `social link` | not in top 100 | `social_link`, `social_links_generator`, `appinio_social_share`, `social_sharing_plus`, `social_share` |
| `social media link` | not in top 100 | `social_link`, `social_links_generator`, `appinio_social_share`, `social_sharing_plus`, `custom_social_share` |
...
| `open whatsapp from flutter` | #1 p1 | DeeplinkX leads |
| `open youtube flutter` | #1 p1 | DeeplinkX leads |
| `open youtube video flutter` | #1 p1 | DeeplinkX leads |
| `open YouTube video from Flutter` | #4 p1 | `iconify_flutter_plus`, `iconify_flutter`, `flutter_boxicons`, `deeplink_x`, `adaptive_video_player` |
| `send whatsapp message flutter` | not in top 100 | `flutter_open_whatsapp`, `whatsapp`, `flutter_chatflow`, `whatsapp_sender_flutter`, `whatsapp_direct_send` |
...
| `google maps directions flutter` | #10 p1 | `flutter_google_maps_webservices`, `googlemaps_flutter_webservices`, `google_routes_flutter`, `google_map_service`, `map_launcher` |
| `Google Maps URL Launcher Flutter` | not in top 100 | `launchify`, `maps_launcher`, `map_launcher`, `url_launcher_utils`, `mappls_map_launcher` |
...
| `open Google Maps and show navigation Flutter` | not in top 100 | `iconify_flutter_plus`, `iconify_flutter`, `flutter_tabler_icons`, `jaspr_icons_pack`, `ubicons` |
...
| `open location in google maps flutter` | #2 p1 | `place_picker_google`, `deeplink_x`, `google_maps_widget`, `google_maps_marker_widgets`, `google_maps_place_picker_mb_v2` |
| `open map app flutter` | #4 p1 | `map_picker_free`, `maps_launcher`, `location_picker_flutter_map`, `deeplink_x`, `flutter_map_pmtiles` |
...
| `flutter is app installed` | #80 p8 | `appcheck`, `check_app_version`, `mappls_map_launcher`, `flutter_appstore_detection`, `flutter_appavailability` |
| `flutter launch another app` | not in top 100 | `external_app_launcher`, `external_app_launcher_plus`, `jitsi_meet_wrapper`, `store_launcher_plus`, `flutter_applaunch` |
| `flutter launch app by package name` | #51 p6 | `app_launcher_packagename`, `jitsi_meet_wrapper`, `store_launcher_plus`, `screen_launch_by_notfication`, `auto_launcher` |
| `flutter launch external app` | #13 p2 | `external_app_launcher`, `external_app_launcher_plus`, `launchify`, `app_activity_launcher`, `dropbox_client` |
| `flutter launch installed app` | #35 p4 | `appcheck`, `app_launcher`, `open_mail_launcher`, `mappls_map_launcher`, `apps_utils` |
| `flutter open another app` | not in top 100 | `external_app_launcher`, `external_app_launcher_plus`, `device_preview_plus`, `device_preview`, `iconify_flutter_plus` |
| `flutter open app by package name` | #28 p3 | `openapp_by_package`, `folder_file_saver`, `tabby_flutter_inapp_sdk`, `openpanel_flutter`, `developer_app_list` |
| `flutter open app if installed` | #16 p2 | `open_mail`, `open_mail_launcher`, `device_apps_plus`, `is_pirated`, `flutter_appavailability` |
| `flutter open app with package name` | #14 p2 | `admob_kit_x`, `change_app_package_name_plus`, `openapp_by_package`, `open_file`, `open_filex` |
...
| `launch other app from Flutter app` | #56 p6 | `bugsee_flutter`, `flutter_alarm_background_trigger`, `android_vlc_player`, `reclaim_sdk`, `bring_app_to_foreground` |
...
| `flutter deep link to another app` | not in top 100 | `uni_links3`, `uni_links2`, `uni_links5`, `uni_links_nfc_support`, `flutter_uni_links` |
| `flutter deeplink to another app` | not in top 100 | `fdb`, `navi`, `smart_splash_kit`, `pilotpm_engage` |
...
| `flutter launch url scheme` | #29 p3 | `url_launcher`, `launchify`, `flutter_applaunch`, `enhanced_url_launcher`, `flutter_custom_tabs` |
| `flutter open universal link` | not in top 100 | `app_links`, `whatsapp_unilink`, `link_bridge`, `flutter_link_nav`, `uni_links3` |
| `flutter open url scheme` | #9 p1 | `neplox_linkpreviewer`, `app_links`, `maps_launcher`, `url_launcher`, `external_app_launcher` |
...
| `flutter open play store` | #8 p1 | `flutter_app_store`, `in_app_update_flutter`, `store_redirect`, `launch_app_store`, `open_appstore` |
| `flutter open store listing` | #4 p1 | `app_review`, `app_review_plus`, `flutter_store_listing`, `deeplink_x`, `is_pirated` |
| `flutter play store link` | #3 p1 | `multi_updater`, `in_app_update_flutter`, `deeplink_x`, `flutter_update_checker`, `billing_country` |
...
| Keyword | Rank | Top visible packages ahead or around it |
| --- | ---: | --- |
| `app links alternative flutter` | not in top 100 | `appsonair_flutter_applink`, `gpt_markdown`, `mailto`, `link_bridge`, `sojo_link` |
| `external app launcher alternative flutter` | not in top 100 | `hzfinger_fingerprint_sdk` |
| `flutter app links alternative` | not in top 100 | `appsonair_flutter_applink`, `gpt_markdown`, `mailto`, `link_bridge`, `sojo_link` |
| `flutter deeplink without url_launcher` | not in top 100 | `neplox_linkpreviewer`, `rgb_cli` |
| `flutter open app without url launcher` | not in top 100 | `launchify`, `url_launcher`, `neplox_linkpreviewer`, `fluwx_no_pay`, `route_pilot` |
...
| Keyword | Rank | Top visible packages ahead or around it |
| --- | ---: | --- |
| `create Threads post Flutter` | not in top 100 | `flutter_angle`, `super_icons`, `flutter_web_gl`, `vad`, `art_adk` |
...
| `open Facebook page Flutter` | #17 p2 | `iconify_flutter_plus`, `iconify_flutter`, `remixicon`, `flutter_boxicons`, `flutter_remix` |
...
| `open Slack user Flutter` | not in top 100 | `iconify_flutter_plus`, `iconify_flutter`, `remixicon`, `flutter_boxicons`, `flutter_remix` |
| `open telegram profile flutter` | #1 p1 | DeeplinkX leads |
| `open Threads comments Flutter` | not in top 100 | `docudart`, `flutter_web_gl`, `gvl_comments`, `font_awesome_flutter`, `font_awesome_icon_class` |
...
| `open Threads topic Flutter` | not in top 100 | no pub.dev API results in scan |
| `open Threads topic from Flutter` | not in top 100 | no pub.dev API results in scan |
| `search Threads Flutter` | #20 p2 | `flutter_chats_ui`, `flutter_advanced_chat_ui`, `remixicon`, `font_awesome_flutter`, `flutter_tabler_icons` |
| `Threads topic tag Flutter` | not in top 100 | no pub.dev API results in scan |
...
| `open TikTok video Flutter` | #13 p2 | `snap_reels`, `iconify_flutter_plus`, `iconify_flutter`, `remixicon`, `flutter_boxicons` |
...
| `search Twitter Flutter` | #38 p4 | `canopas_country_picker`, `twitter_api_v2`, `iconify_flutter_plus`, `iconify_flutter`, `search_bar_animated` |
...
| `WhatsApp phone number Flutter` | not in top 100 | `otp_phone_verify`, `app_contact_checker`, `flutter_dialpad_plus`, `smart_phone_number`, `iconify_flutter_plus` |
...
| `Amap coordinates Flutter` | not in top 100 | `amap_native_plugin`, `qorvia_maps_sdk`, `scoova_maps`, `flutter_heat_map`, `google_maps_extractor` |
| `Amap directions Flutter` | #4 p1 | `map_launcher`, `ola_maps_flutter`, `mappls_map_launcher`, `deeplink_x`, `flutter_google_maps_webservices` |
| `Amap search Flutter` | #21 p3 | `xue_hua_gaode_map`, `csp_amap_flutter_base`, `amap_flutter_search`, `ai_amap`, `kw_amap_search` |
...
| `Baidu Maps search Flutter` | #2 p1 | `flutter_tabler_icons`, `deeplink_x`, `flutter_baidu_mapapi_search`, `jaspr_icons_pack`, `jaspr_icons` |
...
| `Neshan search Flutter` | #3 p1 | `neshan_maps_flutter`, `neshanmap_flutter`, `deeplink_x`, `flutter_wechate_share_plugin`, `opensubtitles_hasher` |
...
| `view location 2GIS Flutter` | #3 p1 | `arcgis_maps_toolkit`, `arcgis_maps`, `deeplink_x`, `gis_mapping`, `mappls_map_launcher` |
| `view location Amap Flutter` | #16 p2 | `ola_map_flutter`, `amap_native_plugin`, `flutter_tamap_location`, `amap_location`, `xbr_gaode_navi_amap` |
| `view location Apple Maps Flutter` | #21 p3 | `apple_maps_flutter`, `apple_maps_place_picker`, `xgh_location_picker_ios`, `iconify_flutter_plus`, `iconify_flutter` |
| `view location Baidu Maps Flutter` | #8 p1 | `flutter_tabler_icons`, `jaspr_icons_pack`, `jaspr_icons`, `tabler_icons_for_flutter`, `flutter_pas_tabler_icons` |
| `view location Google Maps Flutter` | #10 p1 | `webview_location`, `map_location_picker`, `map_flutter`, `flutter_google_maps_widget_cluster_markers`, `cirrus_map_view` |
| `view location Moovit Flutter` | #1 p1 | DeeplinkX leads |
| `view location Neshan Flutter` | #3 p1 | `neshan_maps_flutter`, `neshanmap_flutter`, `deeplink_x`, `flash_animation` |
| `view location Sygic Flutter` | #1 p1 | DeeplinkX leads |
| `view location Waze Flutter` | #3 p1 | `flutter_tabler_icons`, `tabler_icons_flutter`, `deeplink_x`, `jaspr_icons_pack`, `jaspr_icons` |
| `view location Yandex Maps Flutter` | #7 p1 | `flutter_tabler_icons`, `jaspr_icons_pack`, `jaspr_icons`, `flutter_pas_tabler_icons`, `yandex_map_desktop` |
...
| `Amap current location Flutter` | #12 p2 | `ola_map_flutter`, `ultra_map_place_picker`, `amap_native_plugin`, `flutter_map_location_marker`, `ola_maps_flutter` |
| `Amap route Flutter` | not in top 100 | `ola_maps_flutter`, `amap_native_plugin`, `qorvia_maps_sdk`, `google_maps_widget`, `vietmap_flutter_navigation` |
...
| `Waze marker Flutter` | not in top 100 | `map_launcher`, `mappls_map_launcher`, `font_awesome_flutter`, `font_awesome_icon_class`, `opticore` |
...
| Keyword | Rank | Top visible packages ahead or around it |
| --- | ---: | --- |
| `2 GIS` | #49 p5 | `win32_registry`, `geoengine`, `arcgis_maps`, `arcgis_map_sdk`, `flutter_map_arcgis` |
| `2 GIS deeplink` | #2 p1 | `push_message_register`, `deeplink_x` |
| `2 GIS Flutter` | #31 p4 | `arcgis_maps`, `arcgis_map_sdk`, `flutter_map_arcgis`, `flutter_form_registry`, `mapmyindia_flutter` |
...
| `BaiduMaps` | #30 p3 | `flutter_baidu_mapapi_search`, `flutter_baidu_mapapi_utils`, `flutter_baidu_mapapi_base`, `flutter_baidu_mapapi_map`, `map_launcher` |
| `BaiduMaps deeplink` | #1 p1 | DeeplinkX leads |
| `BaiduMaps Flutter` | #31 p4 | `flutter_baidu_mapapi_search`, `flutter_baidu_mapapi_utils`, `flutter_baidu_mapapi_base`, `flutter_baidu_mapapi_map`, `map_launcher` |
...
| `TwoGis` | #54 p6 | `pinput`, `diffutil_dart`, `string_similarity`, `two_dimensional_scrollables`, `geocode` |
...
| `TwoGis link` | #33 p4 | `linked_scroll_controller`, `blinker`, `any_link_connect`, `linkedin_login`, `expandable_text` |
| `yandexmaps` | #8 p1 | `yandex_mapkit`, `yandex_maps_mapkit_lite`, `yandex_maps_mapkit`, `yandex_maps_navikit`, `yandex_mapkit_lite` |
...
| Keyword | Rank | Top visible packages ahead or around it |
| --- | ---: | --- |
| `app to app deeplink Flutter` | #9 p1 | `app_links`, `flutter_branch_sdk`, `google_ads_deferred_deep_link`, `urlynk_flutter`, `deep_link_orchestrator` |
...
| `launch app by application ID` | not in top 100 | `iconify_flutter_plus`, `iconify_flutter`, `win32`, `cnativeapi`, `tizen_app_control` |
| `open app by package name` | #47 p5 | `openapp_by_package`, `openfoodfacts`, `folder_file_saver`, `flutter_animated_icons`, `tabby_flutter_inapp_sdk` |
| `open native app Flutter` | #37 p4 | `open_file`, `open_filex`, `better_open_file`, `square_in_app_payments`, `open_file_plus` |
| `open specific app Flutter` | not in top 100 | `sbp_payments`, `flutter_full_screen_intent`, `iconify_flutter_plus`, `iconify_flutter`, `flutter_document_picker` |
...
| `choose map app Flutter` | #11 p2 | `flutter_map_polywidget`, `ultra_map_place_picker`, `safe_json_mapper`, `geomap_package`, `plug_location_map` |
...
| `default map app Flutter` | not in top 100 | `map_picker`, `mapsindoors_mapbox`, `flutter_map_polywidget`, `google_places_autocomplete`, `mapsindoors_googlemaps` |
...
| `start navigation to coordinates Flutter` | not in top 100 | `flutter_web_gl`, `magiclane_maps_flutter`, `maps_tracking_toolbox`, `flutter_left_right_container`, `location_webview` |

### WhatsApp Intent

| Keyword | Rank | Top visible packages ahead or around it |
| --- | ---: | --- |
| `open WhatsApp chat with phone number` | not in top 100 | `iconify_flutter_plus`, `iconify_flutter`, `jaspr_icons_pack`, `ubicons`, `jaspr_icons` |
| `share text to WhatsApp Flutter` | not in top 100 | `flutter_social_share_plugin`, `flutter_share_me`, `whatsapp_share_plus`, `whatsapp_file_share`, `social_sharing_plus` |
| `wa.me link Flutter` | not in top 100 | `wa_me`, `watermelon_mediator`, `payment_gateway_flutter`, `flutter_web_gl`, `awareframework_core` |
| `WhatsApp Click to Chat Flutter` | not in top 100 | `iconify_flutter_plus`, `iconify_flutter`, `super_icons`, `jaspr_icons_pack`, `ubicons` |
...
| `LaunchMode.externalApplication` | not in top 100 | `app_runner`, `bubble_head`, `ussd_launcher`, `flutter_app_icon_changer`, `vwo_insights_flutter_sdk` |
| `market details Flutter` | not in top 100 | `whatsapp_otp`, `maimaps_flutter`, `cafebazaar_market`, `flutter_bubble_chart`, `flutter_map_dragmarker` |
...
| `type safe app launcher` | not in top 100 | `launchify`, `near_dart`, `nanc_icons`, `route_pilot`, `mappls_map_launcher` |
| `typed external app links` | #1 p1 | DeeplinkX leads |
| `typed URL launcher Flutter` | not in top 100 | `route_pilot`, `url_launcher_utils`, `geniuslink_design_system`, `flutter_local_ai`, `river_cli` |
...
| `Google Play link Flutter` | #1 p1 | DeeplinkX leads |
...
| `available maps in mobile device Flutter` | not in top 100 | `iconify_flutter_plus`, `iconify_flutter`, `stac`, `stac_core`, `jaspr_icons_pack` |
...
| `deep link without package Flutter` | not in top 100 | `my_deep_link_sdk`, `deep_link_manager`, `flutter_facebook_app_links_spm`, `smart_deeplink_router`, `stack_deferred_link` |
...
| `Flutter deep linking and universal linking` | not in top 100 | `deeplink_sdk`, `wildlinks_flutter_sdk`, `deep_link_orchestrator`, `flutter_linkme_sdk`, `flutter_deep_linker` |
| `Flutter Map Launcher` | not in top 100 | `map_launcher`, `maps_launcher`, `flutter_launcher_plus`, `mappls_map_launcher`, `url_launcher` |
...
| `given coordinates Flutter maps` | not in top 100 | `flutter_simple_map`, `open_route_service`, `mapwize`, `termui_flutter`, `fl_geocoder` |
| `Google Maps & URL Launcher in Flutter` | not in top 100 | `launchify`, `map_launcher`, `url_launcher_utils`, `mappls_map_launcher`, `launcher_utils` |
| `Google Maps and URL Launcher in Flutter` | not in top 100 | `launchify`, `map_launcher`, `url_launcher_utils`, `mappls_map_launcher`, `launcher_utils` |
| `Google Maps URL Launcher Flutter` | not in top 100 | `launchify`, `maps_launcher`, `map_launcher`, `url_launcher_utils`, `mappls_map_launcher` |
| `iOS URL scheme Android intent Flutter` | #91 p10 | `appscheme`, `link_bridge`, `uni_links3`, `upi_intent`, `uni_links2` |
| `launch Google Maps directions Flutter` | #4 p1 | `map_launcher`, `mappls_map_launcher`, `iconify_flutter_plus`, `deeplink_x`, `iconify_flutter` |
...
| `launch other app from Flutter app` | #56 p6 | `bugsee_flutter`, `flutter_alarm_background_trigger`, `android_vlc_player`, `reclaim_sdk`, `bring_app_to_foreground` |
| `navigation beyond the app Flutter` | not in top 100 | `bouncy_background`, `idto_flutter`, `sint`, `ui_commenter`, `apptomate_custom_appbar` |
| `open another app without url_launcher Flutter` | not in top 100 | `dynamic_ui_renderer`, `flutter_quill_syncme` |
| `open directly inside app not browser Flutter` | not in top 100 | `maplibre_gl`, `openidconnect`, `omnyshell_web`, `cartographer`, `dangi_doctor` |
...
| `open WhatsApp directly from Flutter` | #2 p1 | `whatsapp`, `deeplink_x`, `whatsapp_direct_send`, `native_social_share`, `media_launcher_plugin` |
| `open YouTube video from Flutter` | #4 p1 | `iconify_flutter_plus`, `iconify_flutter`, `flutter_boxicons`, `deeplink_x`, `adaptive_video_player` |
| `pre-filled WhatsApp message Flutter` | not in top 100 | `preferred_upi_launcher`, `whatsapp_direct_send`, `launcher_utils`, `universal_share`, `social_sender_whatsapp` |
| `redirect users to App Store Play Store` | #12 p2 | `store_redirect`, `store_redirect2`, `win32`, `flutter_app_update_lib`, `in_app_review` |
| `send whatsapp message flutter` | not in top 100 | `flutter_open_whatsapp`, `whatsapp`, `flutter_chatflow`, `whatsapp_sender_flutter`, `whatsapp_direct_send` |
...
| `without url_launcher Flutter deeplink` | not in top 100 | `neplox_linkpreviewer`, `rgb_cli` |

## Recommendations

- Preserve the current description and topics; exact deeplink, typed API, store fallback, and provider-specific searches remain the strongest surfaces.
- Add durable Threads wording for search and topic tags in high-signal documentation. The new action scan shows whether pub.dev has begun associating those human phrases with the package.
- Keep strengthening map-choice and coordinate language such as "find installed maps", "choose map app", "open coordinates in maps", "show marker", and "start navigation"; generic map task intent still trails provider-specific terms.
- Strengthen WhatsApp task wording around Click to Chat, `wa.me`, pre-filled messages, sharing text, and direct send while retaining the exact `whatsapp deeplink` wording that currently performs well.
- Keep explicit comparisons with `whatsapp_unilink`, `map_launcher`, `maps_launcher`, `external_app_launcher`, `app_links`, and `url_launcher` in durable docs or articles; alternative/comparison searches remain difficult to rank from pubspec metadata alone.
- Preserve regional store and navigation-provider pages. Exact provider and fallback terms continue to outperform broad generic intent.
- Treat likes and downloads as positive authority movement, but base the next metadata/content pass on repeated gains across snapshots rather than a single-day position change.
